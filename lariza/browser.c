           ...
           gboolean focus_tab)
{
    ...
    if (uri != NULL && cooperative_instances && !cooperative_alone)
    {
        ...
        {
            fprintf(stderr, __NAME__": Could not write command '%s'\n", f);
        }
        g_free(f);
        return NULL;
    }
    ...
    webkit_settings_set_enable_smooth_scrolling(webkit_web_view_get_settings(WEBKIT_WEB_VIEW(c->web_view)), !disable_smooth_scrolling);
...
grab_environment_configuration(void)
{
    ...
    e = g_getenv(__NAME_UPPERCASE__"_DISABLE_SMOOTH_SCROLLING");
    if (e != NULL)
...
init_default_web_context(void)
{
    ...
    /* FIXME
     *
     * As of WebKit 2.34, sandboxing is broken. It complains about this,
     * then crashes:
     *
     * GApplication is required for xdg-desktop-portal access in the WebKit sandbox.
     *
     * As a quick workaround, disable sandboxing (this is bad), until I
        ...
        if (((GdkEventKey *)event)->state & GDK_CONTROL_MASK)
        {
            switch (((GdkEventKey *)event)->keyval)
            {
                ...
                case GDK_KEY_3:  /* search backward (left hand) */
                    search(c, -1);
                    return TRUE;
                case GDK_KEY_p:  /* location (BOTH hands) */
                    gtk_widget_grab_focus(c->location);
                    return TRUE;
                case GDK_KEY_v:  /* initiate search (BOTH hands) */
                    gtk_widget_grab_focus(c->location);
                ...
                case GDK_KEY_c:  /* reload trusted certs (left hand) */
                    trust_user_certs(wc);
                    return TRUE;
                case GDK_KEY_i:  /* go one tab to the left (left hand) */
                    gtk_notebook_prev_page(GTK_NOTEBOOK(mw.notebook));
                    return TRUE;
                case GDK_KEY_o:  /* go one tab to the right (left hand) */
                    gtk_notebook_next_page(GTK_NOTEBOOK(mw.notebook));
                    return TRUE;
                case GDK_KEY_h: /* navigate backward (left hand) */
                    webkit_web_view_go_back(WEBKIT_WEB_VIEW(c->web_view));
                    return TRUE;
                case GDK_KEY_l: /* navigate forward (left hand) */
